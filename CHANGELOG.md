...
- Шаблон `DateTimeRangeSelector` теперь размещает панель кнопок пресетов в одной строке с панелями `From` и `To` при горизонтальной ориентации (`Orientation = Horizontal`). Строка выравнивается по центру родительского контейнера, между элементами сохраняются отступы по 15 пикселей. В вертикальном режиме пресеты остаются сверху над `From` с отступом 10 пикселей. Динамическое переключение макета реализовано с помощью `Interaction.Behaviors` (`DataTriggerBehavior` + `ChangePropertyAction`), что позволяет сохранить логику полностью декларативной в XAML.
