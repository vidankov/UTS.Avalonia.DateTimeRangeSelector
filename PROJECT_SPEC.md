...
- **Приведение с учётом намерения** (`From` <-> `To` <-> границ Min/Max <-> программного задания) определяет, как восстанавливается условие **From ≤ To** при пересечении конечных точек.
