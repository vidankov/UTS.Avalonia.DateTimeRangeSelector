...
- **Приведение с учётом намерения** (`From` <-> `To` <-> границ Min/Max <-> программная установка) определяет, как восстанавливается условие **From ≤ To** при пересечении конечных точек.
